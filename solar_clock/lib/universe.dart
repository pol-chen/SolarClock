...
class Twinkle {
  const Twinkle({@required this.left, @required this.top, @required this.type});
...
class _UniverseState extends State<Universe> with TickerProviderStateMixin {
  ...
  void initTwinkleAnimation() {
    ...
    for (int i = 0; i < count; i++) {
      Animation fadeAnimation =
          Tween(begin: tweens[i ~/ 2], end: tweens[(count - i - 1) ~/ 2])
              .animate(CurvedAnimation(
                  parent: _twinkleController, curve: intervals[i % 2]));
    ...
    _sizeAnimation = Tween(begin: 0.0, end: 3.5).animate(
  ...
  void initBackgroundAnimation() {
    ...
    _colorAnimationStart =
        ColorTween(begin: Color(0xFF33597A), end: Color(0xFF2B669B))
            .animate(_backgroundController);
    _colorAnimationEnd =
        ColorTween(begin: Color(0xFF152440), end: Color(0xFF081F4A))
            .animate(_backgroundController);
  ...
  Widget twinkleStack() {
    ...
    for (int i = 0; i < _twinkleCount; i++) {
      widgets.add(twinkleWidget(
          _twinkleList[i].left, _twinkleList[i].top, _twinkleList[i].type));
    }
